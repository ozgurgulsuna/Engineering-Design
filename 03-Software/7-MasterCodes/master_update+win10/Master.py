...
def main():
    ...
    while True:
        for i in range(3):
            ...
            if move_x_array[i] == -99999 or move_y_array[i] == -99999:
                move_x_array=[0,0,0]
                move_y_array=[0,0,0]    
...
def get_xy(background_image, background_image_r):
    ...
    if (np.mean(image) < 50 or np.mean(image) > 200) & (np.mean(image_r) < 50 or np.mean(image_r) > 200):
    ...
    try:
            ...
            rect_points = np.array([0,0,0,0],dtype=np.int32)
    #print("Rect2")
    try:
            ...
            rect_points_r = np.array([0,0,0,0],dtype=np.int32)

...
#vid = cv.VideoCapture()                                                # 0 : webcam, to find other cameras, change the number
#vid.open("/dev/v4l/by-id/usb-Alcor_Micro__Corp._USB2.0_WebCamera-video-index1")
#vid_r = cv.VideoCapture()                                                # 0 : webcam, to find other cameras, change the number
#vid_r.open("/dev/v4l/by-id/usb-Alcor_Micro__Corp._USB2.0_WebCamera-video-index0")
#vid.set(3,160)
#vid.set(4,120)
...
while(1):
    ...
    # Capture a frame
    ret, image = vid.read()
    ret2, image_r = vid_r.read()
    if ret2:
        cv.imshow('Live Video2', image_r)
    else:
        print("Error 2")
    if ret:

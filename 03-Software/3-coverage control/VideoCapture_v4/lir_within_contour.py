...
def largest_interior_rectangle(grid, contour):
    ...
    s_map = create_maps(adjacencies, contour)
...
def spans_all_directions(h_vectors, v_vectors): ### her bir yön için span çıkarıyor (toplamda 4 tane)
...
def get_xy_array(x, y, spans, mode=0):###sol üst köşe bulma fonksiyonu
...
def create_maps(adjacencies, contour):
    h_left2right, h_right2left, v_top2bottom, v_bottom2top = adjacencies ##1'leri topladık sol/sağ/yukarı/aşağı vs...

    shape = h_left2right.shape
    span_map = np.zeros(shape + (2,), "uint32")###shape[0]((y)) adet matrix 2xshape[1]((x))'lik(x,y)
    #direction_map = np.zeros(shape, "uint8")
    #saddle_candidates_map = np.zeros(shape, "bool_")

    for idx in prange(len(contour),nogil=True):
        x, y = contour[idx, 0], contour[idx, 1] #x,y of a point of contour
        ...
        span_arrays = spans_all_directions(h_vectors, v_vectors) #check source code
        #n = get_n_directions(span_arrays)
        #direction_map[y, x] = n
        xy_arrays = get_xy_arrays(x, y, span_arrays)##solt üst köşe bulma fonksiyonu
        for direction_idx in range(4):
            ...
            for span_idx in range(span_array.shape[0]):
                ...
                # Why not check ther biggest rectangle here, but store 
                # every one of them in a large array (i.e. span_map)?????
                """
                N WILL ALWAYS BE 1, WHY CHECK IT?
                if n == 3 and not cell_on_contour(x, y, contour):
                    saddle_candidates_map[y, x] = True
                """

    return span_map #,direction_map->ikinci sırada but not usedfor now, , saddle_candidates_map --> do not need
